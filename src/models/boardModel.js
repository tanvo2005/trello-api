...
const getDetails = async (id) => {
  try {
    ...
    return result[0] || null // trả về kết quả đầu tiên của mảng hoặc 1 object rỗng nếu không có kết quả
  } catch (error) { throw new Error(error) }
}

// pushColumnOrderIds dùng để push 1 giá trị columnId vào cuối mảng ColumnOrderIds
const pushColumnOrderIds = async (column) => {
  try {
    const result = await GET_DB().collection(BOARD_COLLECTION_NAME).findOneAndUpdate(
      { _id: new ObjectId(column.boardId) }, // tìm kiếm board dựa vào boardId của column
